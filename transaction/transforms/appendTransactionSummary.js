...
module.exports = {
  ...
  async transform({ batch, allTransactions }) {
    ...
    allTransactions.forEach((d) => {
      ...
      personIdMap[d.person_id] = personIdMap[d.person_id]
      || { person_id: d.person_id, transactions: [] };

      personIdMap[d.person_id].transactions.push(d);
    });

    batch.forEach((person) => {
      Object.entries(placeholder).forEach(([k, v]) => { person[`transaction.${k}`] = v; });
      ...
      Object.entries(output).forEach(([k, v]) => { person[`transaction.${k}`] = v; });
    });
  },
};
